...
int test_reshape_and_cache_flash_basic(DeviceManager& dm, TensorFactory& tf) {
    ...
    at::Tensor slot_mapping = at::arange(NUM_TOKENS, at::TensorOptions().dtype(at::kLong).device(dm.get_device()));
    dm.synchronize();
...
int test_reshape_and_cache_flash_partial(DeviceManager& dm, TensorFactory& tf) {
    ...
    at::Tensor slot_mapping = at::arange(NUM_TOKENS / 2, at::TensorOptions().dtype(at::kLong).device(dm.get_device()));
    dm.synchronize();
